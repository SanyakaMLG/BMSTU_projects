...
type Client struct {
	...
	curX   int
	curY   int
	sum    float64 // Количество полученных от клиента дробей
}
	...
	case "add":
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &point); err != nil {
				...
				var x float64
				first, err1 := strconv.Atoi(point.X)
				second, err2 := strconv.Atoi(point.Y)
				if err1 != nil || err2 != nil {
					errorMsg = "malformed data field"
				} else {
					x = math.Sqrt(math.Pow(math.Abs(float64(first-client.curX)), 2) + math.Pow(math.Abs(float64(second-client.curY)), 2))
					client.curX = first
					client.curY = second
					client.logger.Info("performing addition", "value", x)
					client.sum += x
				}
			}
		}
